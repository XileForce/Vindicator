...
static unsigned int custom_max_freq = 0;
module_param(custom_max_freq, uint, 0644);
...
(struct cpufreq_policy *policy, char *buf)
{
	if (custom_max_freq) return sprintf(buf, "%u\n", custom_max_freq);
...
(struct cpufreq_policy *policy, const char *buf, size_t count)
{
	...
	if (custom_max_freq) new_policy.max = min(new_policy.max, custom_max_freq);
        policy->user_policy.max = new_policy.max;
